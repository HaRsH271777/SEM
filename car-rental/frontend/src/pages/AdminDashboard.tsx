...
import {
  ...
  Filter, Hash, CircleDot, ChevronDown,
} from 'lucide-react';
...
function StatCard({ label, value, subValue, icon: Icon, trend, trendUp, color }: {
  ...
  return (
    <div className="bg-white border border-gray-200 p-4 flex flex-col justify-between min-h-[120px] rounded-sm shadow-sm">
      <div className="flex items-center justify-between mb-3">
        <span className="text-[12px] font-semibold text-gray-500 uppercase tracking-wider">{label}</span>
        <div className={`w-8 h-8 ${c.iconBg} flex items-center justify-center rounded-sm`}>
      ...
      <div>
        ...
        <div className="flex items-center gap-2 mt-1">
          {trend && (
            <span className={`flex items-center gap-0.5 text-[11px] font-medium ${trendUp ? 'text-green-600' : 'text-red-600'}`}>
              {trendUp ? <ArrowUpRight className="w-3 h-3" /> : <ArrowDownRight className="w-3 h-3" />}
              {trend}
            </span>
          )}
          {subValue && <span className="text-[11px] text-gray-500">{subValue}</span>}
        </div>
      </div>
    </div>
  );
...
function ChartTooltip({ active, payload, label }: any) {
  ...
  return (
    <div className="bg-white border border-gray-200 px-3 py-2 shadow-sm text-left">
      <p className="text-[11px] text-gray-500 mb-1">{label}</p>
      {payload.map((p: any, i: number) => (
        <p key={i} className="text-[12px] font-medium text-gray-900">
  ...
  return (
    <div className="bg-white border border-gray-200 overflow-hidden shadow-sm">
      <div className="overflow-x-auto">
        <table className="w-full text-[13px] text-gray-900">
          <thead>
            <tr className="border-b border-gray-200 bg-gray-50">
              {headers.map((h, i) => (
                <th key={i} className="px-4 py-2 hover:bg-gray-100 cursor-pointer text-left text-[12px] font-semibold text-gray-700">{h}</th>
              ))}
            </tr>
          </thead>
...
export default function AdminDashboard() {
  ...
  /* ── Nav config ────────────────────────── */
  const navItems: { key: Tab; label: string; icon: React.ComponentType<{ className?: string }>; badge?: number }[] = [
  ...
  return (
    <div className="flex min-h-[calc(100vh-64px)]">
      <Sidebar
        ...
        initials={(currentUser?.name?.[0] || 'A').toUpperCase()}
        profileName={currentUser?.name || 'Administrator'}
        profileRole="System Administrator"
      />

      {/* ─── Main Content ────────────────────── */}
      <main className="flex-1 overflow-y-auto pb-8 pt-20 md:pt-0">
        {/* Top bar */}
        <div className="sticky top-0 z-30 bg-white shadow-sm">
          <div className="flex items-center justify-between px-6 py-3">
            ...
            <div className="flex items-center gap-2 text-gray-900/80">
              <span className="text-[11px] mr-2">{format(new Date(), 'MMM d, yyyy')}</span>
              <button onClick={getRefreshFn()} className="p-2 hover:text-gray-900 hover:bg-black/10 rounded-sm transition-colors" title="Refresh data">
                <RefreshCw className="w-4 h-4" />
              </button>
              {(activeTab === 'bookings' || activeTab === 'analytics') && (
                <button onClick={handleExportCSV} className="flex items-center gap-1.5 px-3 py-1.5 text-[12px] font-medium text-gray-900 bg-black/10 hover:bg-black/20 rounded-sm transition-colors">
        ...
        <div className="px-6 py-6">
          ...
          {activeTab === 'analytics' && (
            <div className="space-y-6">
              <SectionHeader title="Analytics Overview" subtitle="Key metrics and trends"
                action={<select className="text-[12px] bg-white border border-gray-200 rounded-sm px-3 py-1.5 text-gray-900 focus:outline-none focus:border-azure-blue">
              ...
              {analyticsLoading ? (
                ...
                <>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    {/* Revenue Chart */}
                    <div className="lg:col-span-2 bg-white border border-gray-200 rounded-sm shadow-sm p-4">
                      <div className="flex items-center justify-between mb-4">
                        <div>
                          <h3 className="text-[13px] font-semibold text-gray-900">Revenue Trend</h3>
                          <p className="text-[11px] text-gray-500 mt-0.5">Monthly revenue overview</p>
                        </div>
                        <div className="flex items-center gap-1 text-[11px] text-gray-500 bg-gray-50 rounded-sm px-2 py-1">
                      ...
                      <div className="h-56">
                        <ResponsiveContainer width="100%" height="100%">
                          <AreaChart data={analytics.revenueTrend || []}>
                            ...
                            <Area type="monotone" dataKey="revenue" stroke="#0078D4" fill="url(#colorRev)" strokeWidth={1.5} dot={false} />
                          </AreaChart>
                        </ResponsiveContainer>
                      </div>
                    </div>
                    ...
                    <div className="bg-white border border-gray-200 rounded-sm shadow-sm p-4">
                      <h3 className="text-[13px] font-semibold text-gray-900 mb-1">Status Distribution</h3>
                      <p className="text-[11px] text-gray-500 mb-4">Booking breakdown</p>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                    <div className="lg:col-span-2 bg-white border border-gray-200 rounded-sm shadow-sm p-4">
                      <h3 className="text-[13px] font-semibold text-gray-900 mb-1">Top Performing Vehicles</h3>
                      <p className="text-[11px] text-gray-500 mb-4">Ranked by total bookings</p>
                      <div className="h-52">
                        <ResponsiveContainer width="100%" height="100%">
                          <BarChart data={(analytics.topVehicles || []).slice(0, 6)} barSize={28}>
                            ...
                            <Tooltip content={<ChartTooltip />} />
                            <Bar dataKey="bookings" fill="#0078D4" radius={[4, 4, 0, 0]} />
                          </BarChart>
                        </ResponsiveContainer>
                      </div>
                    </div>
                    ...
                    <div className="space-y-4">
                      {/* Revenue Breakdown */}
                      <div className="bg-white border border-gray-200 rounded-sm shadow-sm p-4">
                        <h3 className="text-[13px] font-semibold text-gray-900 mb-3">Revenue Breakdown</h3>
                        <div className="space-y-3">
                          <div className="flex items-center justify-between">
                            <span className="text-[12px] text-gray-700">GMV</span>
                            <span className="text-[13px] font-semibold text-emerald-600">₹{((analytics.gmv || 0) / 1000).toFixed(1)}k</span>
                          </div>
                          <div className="w-full bg-gray-100 rounded-full h-1.5">
                            <div className="bg-green-500 h-1.5 rounded-full transition-all" style={{ width: `${Math.min(100, ((analytics.commissionRevenue || 0) / Math.max(analytics.gmv || 1, 1)) * 100)}%` }} />
                          </div>
                          <div className="flex items-center justify-between">
                            <span className="text-[12px] text-gray-700">Commission</span>
                            <span className="text-[13px] font-semibold text-blue-600">₹{((analytics.commissionRevenue || 0) / 1000).toFixed(1)}k</span>
                          </div>
                          <div className="flex items-center justify-between pt-2 border-t border-gray-200">
                            <span className="text-[12px] text-gray-700">Cancellation Rate</span>
                            <span className={`text-[13px] font-semibold ${(analytics.cancellationRate || 0) > 15 ? 'text-red-600' : 'text-emerald-600'}`}>
                      ...
                      <div className="bg-white border border-gray-200 rounded-sm shadow-sm p-4">
                        <div className="flex items-center gap-2 mb-3">
                          ...
                          <h3 className="text-[13px] font-semibold text-gray-900">Alerts</h3>
                        </div>
                        {(analytics.fraudAlerts || []).length === 0 ? (
                          ...
                          <div className="space-y-1.5">
                            {(analytics.fraudAlerts || []).slice(0, 4).map((alert: any, i: number) => (
                              <div key={i} className="text-[11px] text-amber-700 bg-amber-50 border border-amber-200 rounded-sm px-2.5 py-1.5">
                      ...
                      {/* Top Cities */}
                      <div className="bg-white border border-gray-200 rounded-sm shadow-sm p-4">
                        <h3 className="text-[13px] font-semibold text-gray-900 mb-3">Top Cities</h3>
                        <div className="space-y-2">
                          {(analytics.topCities || []).slice(0, 4).map((city: any, i: number) => (
                            <div key={i} className="flex items-center justify-between text-[12px]">
                              ...
                              <span className="text-gray-500">{city.bookings}</span>
                            </div>
                          ))}
                          {(analytics.topCities || []).length === 0 && <p className="text-[12px] text-gray-400">No data available</p>}
                        </div>
                        <div className="mt-3 pt-3 border-t border-gray-200 flex items-center justify-between">
                          <span className="text-[11px] text-gray-500">New users (30d)</span>
                          <span className="text-[15px] font-semibold text-gray-900">{(analytics.userGrowth || []).reduce((s: number, g: any) => s + (g.newUsers || 0), 0)}</span>
                        </div>
                      </div>
                    </div>
                  </div>
          ...
          {activeTab === 'bookings' && (
            <div className="space-y-4">
              <SectionHeader title="All Bookings" subtitle={`${bookingsTotal} total bookings`} />

              {/* Filters */}
              <div className="flex flex-wrap gap-2 mb-4">
                {['all', 'pending', 'confirmed', 'active', 'completed', 'cancelled'].map(s => (
                  <button key={s} onClick={() => setBookingsFilter(s)}
                    className={`px-3 py-1.5 text-[11px] font-medium rounded-sm transition-colors capitalize ${
                      bookingsFilter === s ? 'bg-white text-gray-900' : 'bg-white border border-gray-200 text-gray-500 hover:text-gray-900 hover:bg-gray-50'
                    }`}>
                    {s}
                  </button>
                ))}
              ...
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                <div className="relative w-full sm:w-72">
                  ...
                  <input type="text" placeholder="Search bookings..." value={bookingsSearch} onChange={e => setBookingsSearch(e.target.value)}
                    className="w-full pl-9 pr-3 py-2 text-[12px] bg-white border border-gray-200 rounded-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:border-azure-blue focus:ring-1 focus:ring-azure-blue/20 transition-colors" />
                </div>
                    ...
                    {filteredBookings.map(b => (
                      <tr key={b._id} className="hover:bg-gray-50 transition-colors">
                        <td className="px-4 py-3">
                          <span className="font-mono text-[11px] text-gray-600 bg-gray-100 px-1.5 py-0.5 rounded">#{b._id?.slice(-8)}</span>
                        </td>
                        <td className="px-4 py-3 font-medium text-gray-900 text-[13px]">{b.vehicleTitle || '—'}</td>
                        <td className="px-4 py-3 text-gray-600 text-[12px]">{format(new Date(b.startDate), 'MMM d')} – {format(new Date(b.endDate), 'MMM d, yyyy')}</td>
                        <td className="px-4 py-3"><StatusBadge status={b.status} /></td>
                        <td className="px-4 py-3 text-[13px] font-medium text-gray-900">₹{b.priceBreakdown?.total?.toLocaleString() || '—'}</td>
                        <td className="px-4 py-3 text-[12px] text-gray-500">{b.createdAt ? format(new Date(b.createdAt), 'MMM d, yyyy') : '—'}</td>
                      </tr>
                    ))}
                  </TableShell>
                  <div className="flex items-center justify-between pt-2">
                    <p className="text-[12px] text-gray-500">Showing {(bookingsPage - 1) * 20 + 1}–{Math.min(bookingsPage * 20, bookingsTotal)} of {bookingsTotal}</p>
                    <div className="flex items-center gap-1">
                      <button onClick={() => setBookingsPage(p => Math.max(1, p - 1))} disabled={bookingsPage <= 1} title="Previous page"
                        className="p-1.5 text-gray-500 hover:text-gray-900 hover:bg-gray-100 rounded-md disabled:opacity-30 disabled:cursor-not-allowed transition-colors">
                        <ChevronLeft className="w-4 h-4" />
                      </button>
                      <span className="px-2 py-1 text-[12px] text-gray-700 bg-gray-100 rounded-md min-w-[32px] text-center">{bookingsPage}</span>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-3">
                          <div className="w-10 h-10 rounded-lg bg-white overflow-hidden flex-shrink-0 border border-gray-200">
                            {v.images?.[0] ? <img src={v.images[0].url} alt="" className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center"><Car className="w-4 h-4 text-gray-400" /></div>}
                          </div>
                          <div>
                            <p className="text-[13px] font-medium text-gray-900">{v.title}</p>
                            <p className="text-[11px] text-gray-500 font-mono">#{v._id?.slice(-6)}</p>
                          </div>
                        </div>
                      </td>
                      <td className="px-4 py-3 text-[12px] text-gray-500">{v.location || '—'}</td>
                      <td className="px-4 py-3 text-[13px] font-medium text-gray-900">₹{v.pricing?.baseRate?.toLocaleString()}/d</td>
                      <td className="px-4 py-3"><StatusBadge status={v.status} /></td>
                      <td className="px-4 py-3">
                        {v.status === 'pending_approval' ? (
                          <div className="flex items-center gap-1.5">
                            <button onClick={() => handleApprove(v._id)} className="px-2.5 py-1 text-[11px] font-medium text-green-600 bg-green-500/10 hover:bg-green-500/20 rounded-md transition-colors">Approve</button>
              ...
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                ...
                <div className="relative w-full sm:w-64">
                  ...
                  <input type="text" placeholder="Search users..." value={usersSearch} onChange={e => setUsersSearch(e.target.value)}
                    className="w-full pl-9 pr-3 py-2 text-[12px] bg-white shadow-sm border border-gray-200 rounded-lg text-gray-700 placeholder-[#4b5058] focus:outline-none focus:border-azure-blue/50 focus:ring-1 focus:ring-azure-blue/20 transition-colors" />
                </div>
              </div>
              {usersLoading ? (
                <div className="space-y-2">{Array.from({ length: 4 }).map((_, i) => <BookingRowSkeleton key={i} />)}</div>
                    ...
                    <tr key={u._id} className="hover:bg-gray-50 transition-colors">
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2.5">
                          <div className="w-7 h-7 rounded-full bg-azure-blue/15 flex items-center justify-center text-[11px] font-semibold text-blue-600 flex-shrink-0">{u.name?.charAt(0)?.toUpperCase() || '?'}</div>
                      ...
                      <td className="px-4 py-3 text-[12px] text-gray-500">{u.createdAt ? format(new Date(u.createdAt), 'MMM d, yyyy') : '—'}</td>
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1.5">
                          <div className={`w-1.5 h-1.5 rounded-full ${u.isActive !== false ? 'bg-green-500' : 'bg-[#4b5058]'}`} />
                          <span className="text-[11px] text-gray-500">{u.isActive !== false ? 'Active' : 'Inactive'}</span>
                        </div>
                      </td>
                    </tr>
                  ))}
                ...
                <TableShell headers={['Action', 'Actor', 'Entity', 'Details', 'Time']}>
                  {auditLogs.map(log => (
                    <tr key={log._id} className="hover:bg-gray-50 transition-colors">
                      <td className="px-4 py-3"><span className="text-[12px] font-medium text-gray-900 bg-gray-100 px-2 py-0.5 rounded-md">{log.action}</span></td>
                      <td className="px-4 py-3 text-[12px] text-gray-500">{log.actorEmail || log.actorId?.slice(-6) || '—'}</td>
                      <td className="px-4 py-3 text-[11px] text-gray-500 font-mono">{log.entityType}:{log.entityId?.slice(-6)}</td>
                      <td className="px-4 py-3 text-[11px] text-gray-500 max-w-[220px] truncate">{log.details ? JSON.stringify(log.details) : '—'}</td>
                      <td className="px-4 py-3 text-[11px] text-gray-500">{log.createdAt ? format(new Date(log.createdAt), 'MMM d, HH:mm') : '—'}</td>
                    </tr>
                  ))}
                </TableShell>
              )}
          ...
          {activeTab === 'config' && (
            <div className="space-y-4">
              ...
              {configLoading ? (
                <div className="space-y-2">{[...Array(5)].map((_, i) => <div key={i} className="h-14 bg-white shadow-sm rounded-xl animate-pulse border border-gray-200" />)}</div>
              ) : (
                <div className="bg-white shadow-sm border border-gray-200 rounded-xl divide-y divide-white/[0.04] overflow-hidden">
                  {configs.map(cfg => (
                    <div key={cfg._id} className="flex items-center justify-between px-5 py-4 hover:bg-gray-50 transition-colors">
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          <Hash className="w-3 h-3 text-gray-400" />
                          <p className="text-[13px] font-medium text-gray-900">{cfg.key}</p>
                        </div>
                        {cfg.description && <p className="text-[11px] text-gray-500 mt-0.5 ml-5">{cfg.description}</p>}
                      </div>
                      {editingConfig === cfg.key ? (
                        <div className="flex items-center gap-2">
                          <input type="text" value={configVal} onChange={e => setConfigVal(e.target.value)} autoFocus title="Config value" placeholder="Value"
                            className="w-28 text-[12px] bg-azure-bg border border-azure-blue/30 rounded-md px-2.5 py-1.5 text-gray-900 focus:outline-none focus:border-azure-blue/60" />
                          ...
                          }} className="px-2.5 py-1 text-[11px] font-medium text-green-600 bg-green-500/10 hover:bg-green-500/20 rounded-md transition-colors">Save</button>
                          <button onClick={() => setEditingConfig(null)} className="px-2.5 py-1 text-[11px] text-gray-500 hover:text-gray-700 rounded-md transition-colors">Cancel</button>
                        </div>
                      ) : (
                        <div className="flex items-center gap-3">
                          <span className="text-[13px] font-mono text-blue-600 bg-azure-blue/8 px-2 py-0.5 rounded">{String(cfg.value)}</span>
                          <button onClick={() => { setEditingConfig(cfg.key); setConfigVal(String(cfg.value)); }} className="text-[11px] text-gray-500 hover:text-blue-600 transition-colors">Edit</button>
                        </div>
                      )}
                    </div>
                  ))}
          ...
          {activeTab === 'blacklist' && (
            <div className="space-y-5">
              ...
              {blacklistLoading ? (
                <div className="space-y-2">{[...Array(3)].map((_, i) => <div key={i} className="h-16 bg-white shadow-sm rounded-xl animate-pulse border border-gray-200" />)}</div>
              ) : blacklist.length === 0 ? (
                <EmptyState title="No blacklisted users" description="Users flagged for suspicious activity will appear here." />
              ) : (
                <div className="bg-white shadow-sm border border-gray-200 rounded-xl divide-y divide-white/[0.04] overflow-hidden">
                  {blacklist.map(bl => (
                    <div key={bl._id} className="flex items-center justify-between px-5 py-4 hover:bg-gray-50 transition-colors">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-[13px] font-medium text-gray-900">{bl.user?.name || bl.userId}</p>
                          <p className="text-[11px] text-gray-500">{bl.user?.email} · {bl.reason}</p>
                        </div>
                      </div>
                      <button onClick={async () => {
              ...
              <div className="bg-white shadow-sm border border-gray-200 rounded-xl p-5">
                <h3 className="text-[13px] font-semibold text-gray-900 mb-3">Add to Blacklist</h3>
                <div className="flex flex-col sm:flex-row gap-2.5">
                  <input id="bl-uid" type="text" placeholder="User ID"
                    className="flex-1 text-[12px] bg-azure-bg border border-gray-200 rounded-lg px-3 py-2 text-gray-700 placeholder-[#4b5058] focus:outline-none focus:border-azure-blue/50" />
                  ...
                  }} className="px-4 py-2 text-[12px] font-medium text-gray-900 bg-red-600 hover:bg-red-500 text-gray-900 rounded-lg transition-colors whitespace-nowrap">Blacklist User</button>
                </div>
              </div>
            </div>
          )}
          ...
          {activeTab === 'disputes' && (
            <div className="space-y-4">
              ...
              {disputesLoading ? (
                ...
                <div className="bg-white shadow-sm border border-gray-200 rounded-xl p-10 text-center">
                  <CheckCircle className="w-8 h-8 text-green-600/50 mx-auto mb-3" />
                  <p className="text-[13px] font-medium text-gray-700">All clear</p>
                  <p className="text-[12px] text-gray-500 mt-1">No disputes require attention.</p>
                </div>
              ) : (
                <div className="space-y-2">
                  {disputes.map(d => (
                    <div key={d._id} className="bg-white shadow-sm border border-gray-200 rounded-xl px-5 py-4 hover:bg-gray-50 transition-colors">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          <div className="w-8 h-8 rounded-lg bg-orange-500/10 flex items-center justify-center"><AlertTriangle className="w-3.5 h-3.5 text-orange-600" /></div>
                          <div>
                            <div className="flex items-center gap-2">
                              <p className="text-[13px] font-medium text-gray-900">Booking</p>
                              <span className="font-mono text-[11px] text-gray-500 bg-gray-100 px-1.5 py-0.5 rounded">#{d._id?.slice(-6)}</span>
                            </div>
                            <p className="text-[11px] text-gray-500 mt-0.5">User: {d.userId?.slice(-8)} · Owner: {d.ownerId?.slice(-8)}</p>
                          </div>
                        </div>
                        <div className="flex items-center gap-1.5">
                          <button onClick={async () => {
                          ...
                          }} className="px-3 py-1.5 text-[11px] font-medium text-green-600 bg-green-500/10 hover:bg-green-500/20 rounded-md transition-colors">Refund</button>
                          <button onClick={async () => {
                            try {
                          ...
                          }} className="px-3 py-1.5 text-[11px] font-medium text-orange-600 bg-orange-500/10 hover:bg-orange-500/20 rounded-md transition-colors">Dismiss</button>
                        </div>
                      </div>
                    </div>
                  ))}
          ...
          {activeTab === 'announcements' && (
            <div className="space-y-5">
              ...
              {/* Create form */}
              <div className="bg-white shadow-sm border border-gray-200 rounded-xl p-5">
                <h3 className="text-[13px] font-semibold text-gray-900 mb-4">New Announcement</h3>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div>
                    <label className="text-[11px] font-medium text-gray-500 uppercase tracking-wider mb-1.5 block">Title</label>
                    <input type="text" placeholder="Announcement title" value={newAnn.title} onChange={e => setNewAnn(p => ({ ...p, title: e.target.value }))}
                      className="w-full text-[12px] bg-azure-bg border border-gray-200 rounded-lg px-3 py-2.5 text-gray-700 placeholder-[#4b5058] focus:outline-none focus:border-azure-blue/50" />
                  </div>
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      <label className="text-[11px] font-medium text-gray-500 uppercase tracking-wider mb-1.5 block">Type</label>
                      <select value={newAnn.type} onChange={e => setNewAnn(p => ({ ...p, type: e.target.value }))} title="Announcement type"
                        className="w-full text-[12px] bg-azure-bg border border-gray-200 rounded-lg px-3 py-2.5 text-gray-700 focus:outline-none focus:border-azure-blue/50">
                        {['info', 'warning', 'success', 'error'].map(t => <option key={t} value={t}>{t.charAt(0).toUpperCase() + t.slice(1)}</option>)}
                      </select>
                    </div>
                    <div>
                      <label className="text-[11px] font-medium text-gray-500 uppercase tracking-wider mb-1.5 block">Target</label>
                  ...
                  <div className="sm:col-span-2">
                    <label className="text-[11px] font-medium text-gray-500 uppercase tracking-wider mb-1.5 block">Message</label>
                    <textarea placeholder="Enter your announcement message..." value={newAnn.message} onChange={e => setNewAnn(p => ({ ...p, message: e.target.value }))}
                      className="w-full text-[12px] bg-azure-bg border border-gray-200 rounded-lg px-3 py-2.5 text-gray-700 placeholder-[#4b5058] min-h-[80px] focus:outline-none focus:border-azure-blue/50 resize-none" />
                  </div>
                  <div className="sm:col-span-2 flex justify-end">
                    ...
                    }} className="px-4 py-2 text-[12px] font-medium text-gray-900 bg-blue-600 hover:bg-azure-blue rounded-lg transition-colors flex items-center gap-1.5">
              ...
              {announcementsLoading ? (
                <div className="space-y-2">{[...Array(2)].map((_, i) => <div key={i} className="h-16 bg-white shadow-sm rounded-xl animate-pulse border border-gray-200" />)}</div>
              ) : announcements.length === 0 ? (
                <EmptyState title="No announcements" description="Create an announcement to broadcast to users." />
              ) : (
                <div className="space-y-2">
                  {announcements.map(ann => {
                    const typeStyle = ann.type === 'warning' ? 'border-l-orange-500' : ann.type === 'error' ? 'border-l-red-500' : ann.type === 'success' ? 'border-l-green-500' : 'border-l-blue-500';
                    return (
                      <div key={ann._id} className={`bg-white shadow-sm border border-gray-200 border-l-2 ${typeStyle} rounded-xl px-5 py-4 hover:bg-gray-50 transition-colors`}>
                        <div className="flex items-start justify-between gap-4">
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2 mb-1">
                              <p className="text-[13px] font-medium text-gray-900">{ann.title}</p>
                              <span className={`text-[10px] px-1.5 py-0.5 rounded font-medium uppercase tracking-wider ${
                                ann.type === 'warning' ? 'bg-orange-500/10 text-orange-600' : ann.type === 'error' ? 'bg-red-500/10 text-red-400' : ann.type === 'success' ? 'bg-green-500/10 text-green-600' : 'bg-blue-500/10 text-blue-400'
                              }`}>{ann.type}</span>
                            </div>
                            <p className="text-[12px] text-gray-500 line-clamp-2">{ann.message}</p>
                          ...
                          }} className="p-1.5 text-gray-400 hover:text-red-400 hover:bg-red-500/10 rounded-md transition-colors flex-shrink-0" title="Delete announcement">
