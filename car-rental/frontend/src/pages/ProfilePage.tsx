  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
      ...
      <div className="card p-6">
        ...
        <form onSubmit={handleUpdateProfile} className="space-y-4">
          ...
          <div className="border-t border-gray-200 pt-4">
            <h3 className="text-sm font-semibold text-gray-6000 mb-3 flex items-center gap-2">
      ...
      <div className="card p-6">
        ...
        {referralLoading ? (
          <div className="h-20 bg-white shadow-sm animate-pulse rounded-xl" />
        ) : referral ? (
          <div className="space-y-4">
            <div className="flex items-center gap-3">
              <div className="flex-1 bg-white shadow-sm rounded-xl px-4 py-3 font-mono text-lg text-primary-400 tracking-widest">
            ...
            <div className="grid grid-cols-3 gap-4 text-center">
              <div className="bg-white shadow-sm rounded-xl p-3">
                <p className="text-2xl font-bold text-gray-600">{referral.referralCount}</p>
              ...
              <div className="bg-white shadow-sm rounded-xl p-3">
                ...
                <p className="text-xs text-gray-600 mt-1">Discount Earned</p>
              </div>
              <div className="bg-white shadow-sm rounded-xl p-3">
                <p className="text-2xl font-bold text-green-400">{(referral.referrals || []).length}</p>
                <p className="text-xs text-gray-600 mt-1">Active Refs</p>
              </div>
            </div>
            <p className="text-xs text-gray-600">Share your referral code. Both you and the new user get a discount on their first booking!</p>
          </div>
        ) : (
          <p className="text-sm text-gray-600">Referral information unavailable.</p>
        )}
      </div>
      </ScrollReveal>
      ...
      <div className="card p-6">
        ...
        {verLoading ? (
          <div className="h-16 bg-white shadow-sm animate-pulse rounded-xl mb-4" />
        ) : verifications.length > 0 ? (
          <div className="space-y-2 mb-5">
            {verifications.map(v => (
              <div key={v._id} className="flex items-center justify-between p-3 bg-white shadow-sm rounded-xl">
                <div>
                  <p className="text-sm font-medium text-gray-600 capitalize">{v.type.replace('_', ' ')}</p>
                  <p className="text-xs text-gray-600">{v.createdAt ? new Date(v.createdAt).toLocaleDateString() : ''}</p>
                </div>
          ...
          <p className="text-sm text-gray-600 mb-5">No verifications submitted yet.</p>
        )}

        {/* Submit new verification */}
        <div className="border-t border-gray-200 pt-5 space-y-4">
          <h4 className="text-sm font-medium text-gray-600">Submit New Document</h4>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <div>
              ...
              <input
                ...
                id="ver-upload"
              />
              <label htmlFor="ver-upload" className="input-field flex items-center gap-2 cursor-pointer text-gray-600 hover:text-gray-600">
