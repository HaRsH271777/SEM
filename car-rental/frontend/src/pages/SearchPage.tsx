...
export default function SearchPage() {
  ...
  return (
    <div className="container-app py-8 bg-[#0d0e14] text-white min-h-screen max-w-none pb-24 md:pb-8 relative">
      <style>{`
        /* Hide the global Navbar specifically on the Search Page */
        #global-navbar { display: none !important; }
      `}</style>
      
      {/* Search Header / Bar (Animated from Navbar position) */}
      <div 
        className="fixed top-0 left-0 w-full z-[9999] px-4 pt-4 sm:pt-6 flex justify-center animate-ripple-reveal pointer-events-none"
      >
        <div className="pointer-events-auto w-full max-w-5xl rounded-full bg-[#1b1b1b] shadow-xl shadow-black/5 border border-gray-800 p-2 flex items-center gap-2">
          
          {/* Logo return button */}
          <button onClick={() => window.location.href = '/'} className="px-4 py-2 text-[15px] font-medium text-white shrink-0">
            DriveX Pro
          </button>
          ...
          <div className="flex-1 relative">
            ...
            <input
              ...
              className="w-full bg-[#252525]/50 hover:bg-[#252525] border border-transparent rounded-full pl-10 pr-4 py-2 text-[14px] text-white placeholder:text-gray-500 focus:outline-none focus:bg-[#1b1b1b] focus:border-primary-500/50 focus:ring-4 focus:ring-primary-500/10 transition-all"
            />
          </div>
          ...
          <div className="shrink-0 flex items-center gap-1.5 pr-2">
            <button
              ...
              className={"flex items-center gap-2 px-3 py-2 rounded-full border text-sm font-medium transition-colors " + (activeFilterCount > 0 ? 'bg-primary-900/40 border-primary-200 text-primary-400' : 'bg-[#1b1b1b] hover:bg-[#252525] border-gray-800 text-gray-400')}
            >
              <SlidersHorizontal className="w-4 h-4" />
              <span className="hidden sm:inline">Filters</span>
              {activeFilterCount > 0 && <span className="bg-primary-500 text-white text-[10px] rounded-full w-4 h-4 flex items-center justify-center font-bold">{activeFilterCount}</span>}
            </button>
            <button
              onClick={() => setViewMode(v => v === 'grid' ? 'map' : 'grid')}
              className="p-2 bg-[#1b1b1b] hover:bg-[#252525] border border-gray-800 rounded-full text-gray-400 transition-colors"
      ...
      <div className="pt-24">{/* Pushed down content to account for sticky search header */}</div>

      {/* Quick Toggles */}
      <div className="flex flex-wrap gap-2 mb-4">
        <button
          onClick={() => updateFilter('availableNow', !filters.availableNow)}
          className={"flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-full border transition-colors " + (filters.availableNow ? 'bg-green-600/20 border-green-500 text-green-400' : 'bg-[#1b1b1b] shadow-sm border-gray-800 text-gray-400')}
        >
          <span className={"w-1.5 h-1.5 rounded-full " + (filters.availableNow ? 'bg-green-400' : 'bg-[#1b1b1b] shadow-sm')} />
          Available Now
        </button>
        <button
          onClick={() => updateFilter('instantBooking', !filters.instantBooking)}
          className={"flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-full border transition-colors " + (filters.instantBooking ? 'bg-yellow-600/20 border-yellow-500 text-yellow-400' : 'bg-[#1b1b1b] shadow-sm border-gray-800 text-gray-400')}
        >
        ...
        <div className="relative ml-auto">
          <select
            ...
            className="text-xs bg-[#1b1b1b] shadow-sm border border-gray-800 rounded-full pl-3 pr-7 py-1.5 text-gray-400 appearance-none focus:outline-none cursor-pointer"
          >
        ...
        {/* Location detect */}
        <button onClick={detectLocation} className="flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-full border bg-[#1b1b1b] shadow-sm border-gray-800 text-gray-400 hover:text-gray-400">
          ?? Near Me
        </button>

        {/* Save search */}
        {user && (
          <button onClick={saveCurrentSearch} className="flex items-center gap-1 text-xs px-3 py-1.5 rounded-full border bg-[#1b1b1b] shadow-sm border-gray-800 text-gray-400 hover:text-gray-400">
        ...
        {user && savedSearches.length > 0 && (
          <div className="relative">
            <button
              ...
              className="flex items-center gap-1 text-xs px-3 py-1.5 rounded-full border bg-[#1b1b1b] shadow-sm border-gray-800 text-gray-400 hover:text-gray-400"
            >
              <Clock className="w-3 h-3" /> Saved ({savedSearches.length})
            </button>
            {showSaved && (
              <div className="absolute right-0 top-full mt-1 z-30 min-w-48 bg-[#1b1b1b] shadow-sm border border-gray-800 rounded-xl shadow-xl py-1">
                {savedSearches.map(s => (
                  <div key={s._id} className="flex items-center justify-between px-3 py-2 hover:bg-[#1b1b1b] shadow-sm group">
                    <button onClick={() => loadSavedSearch(s)} className="text-xs text-gray-400 flex-1 text-left">{s.name}</button>
                    <button onClick={() => deleteSavedSearch(s._id)} className="text-gray-4000 hover:text-red-400 ml-2 opacity-0 group-hover:opacity-100">
                      <X className="w-3 h-3" />
                    </button>
                  </div>
                ))}
      ...
      {/* Expanded Filters */}
      {filtersOpen && (
        <div className="bg-[#1b1b1b] shadow-sm border border-gray-800 rounded-2xl p-5 mb-6 grid grid-cols-2 md:grid-cols-4 gap-4">
          <div>
          ...
          <div>
            <label className="block text-xs text-gray-400 mb-1">Min Seats</label>
          ...
          <div>
            <label className="block text-xs text-gray-400 mb-1">Min Price/day (Rs.)</label>
          ...
          <div>
            <label className="block text-xs text-gray-400 mb-1">Max Price/day (Rs.)</label>
          ...
          <div className="col-span-2 md:col-span-4 flex justify-end">
            <button onClick={clearFilters} className="text-xs text-gray-400 hover:text-gray-400 flex items-center gap-1">
      ...
      {loading ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {[...Array(6)].map((_, i) => (
            <div key={i} className="h-64 bg-[#1b1b1b] shadow-sm rounded-2xl animate-pulse" />
          ))}
        </div>
      ) : vehicles.length === 0 ? (
        <div className="text-center py-20">
          <Search className="w-12 h-12 text-dark-600 mx-auto mb-3" />
          <p className="text-gray-400">No vehicles match your criteria</p>
      ...
      {pages > 1 && (
        <div className="flex justify-center gap-2 mt-8">
          ...
          <span className="text-sm text-gray-400 flex items-center">{page} / {pages}</span>
