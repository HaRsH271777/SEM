        ...
        <div className="space-y-6">
          ...
          <div className="flex gap-2">
            {['weekly', 'monthly', 'yearly'].map(r => (
              <button key={r} onClick={() => setAnalyticsRange(r)} className={`text-xs px-3 py-1.5 rounded-full border transition-colors capitalize ${analyticsRange === r ? 'bg-primary-600/20 border-primary-500 text-primary-400' : 'bg-white shadow-sm border-gray-200 text-gray-600'}`}>{r}</button>
            ))}
          </div>

          {analyticsLoading ? (
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">{[...Array(4)].map((_, i) => <div key={i} className="h-24 bg-white shadow-sm rounded-2xl animate-pulse" />)}</div>
          ) : analytics ? (
            <>
              {/* KPI Cards */}
