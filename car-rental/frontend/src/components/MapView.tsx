...
export default function MapView({ vehicles, userLat, userLng }: MapViewProps) {
  ...
  return (
    <div className="relative rounded-2xl overflow-hidden border border-gray-200 h-[520px]">
      {/* Map iframe */}
      <iframe
        src={mapUrl}
      ...
      <div className="absolute inset-0 bg-white shadow-sm pointer-events-none" />
          ...
          return (
            ...
            >
              <div className={`relative flex flex-col items-center transition-transform group-hover:scale-110 ${selected?._id === v._id ? 'scale-110' : ''}`}>
                <div className="bg-primary-500 text-gray-900 text-xs font-bold px-2 py-1 rounded-full shadow-lg whitespace-nowrap border-2 border-white">
                  ₹{v.pricing.baseRate}
                </div>
      ...
      {userLat && userLng && (
        <div className="absolute" style={{ top: '50%', left: '50%', transform: 'translate(-50%, -50%)', pointerEvents: 'none' }}>
          <div className="bg-blue-500 text-gray-900 w-6 h-6 rounded-full border-2 border-white shadow-lg flex items-center justify-center">
      ...
      {vehicles.length > vehiclesWithGeo.length && (
        <div className="absolute bottom-24 right-3 bg-white shadow-sm rounded-lg px-3 py-1.5 text-xs text-gray-600">
      ...
      {selected && (
        <div className="absolute bottom-4 left-4 right-4 bg-white shadow-sm border border-gray-200 rounded-2xl p-4 shadow-2xl flex gap-3">
          {selected.images[0] && (
            <img
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-start justify-between gap-2">
              <h3 className="font-semibold text-gray-600 text-sm truncate">{selected.title}</h3>
              <button onClick={() => setSelected(null)} className="text-gray-600 hover:text-gray-600 flex-shrink-0 text-lg leading-none">×</button>
            </div>
            ...
            <div className="mt-2 flex items-center justify-between">
              ...
              <button
                ...
                className="text-xs bg-primary-600 hover:bg-primary-500 text-gray-900 px-3 py-1 rounded-lg"
              >
                View
              </button>
            </div>
      ...
      {vehiclesWithGeo.length === 0 && (
        <div className="absolute inset-0 flex items-center justify-center">
          <div className="bg-white shadow-sm rounded-2xl p-6 text-center">
            ...
            <p className="text-gray-600 text-xs mt-1">Owners need to add GPS coordinates</p>
          </div>
        </div>
      )}
    </div>
