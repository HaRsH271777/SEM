...
export default function VehicleCard({ vehicle, onQuickBook }: Props) {
  ...
  return (
    <article
      ...
      {...tiltProps}
      className="group cursor-pointer rounded-3xl overflow-hidden transition-all duration-500 hover:-translate-y-2 relative "
      onClick={handleView}
      role="article"
    ...
    >
      ...
      <div className="absolute inset-0 rounded-3xl border border-gray-200 group-hover:border-gray-200 transition-colors duration-500" />
      ...
      <div className="relative h-48 sm:h-52 overflow-hidden">
        ...
        <div className="absolute top-3 left-3">
          <span className="inline-flex items-center gap-1 bg-white/5 shadow-sm  text-white text-[11px] font-bold px-2.5 py-1 rounded-lg border border-white/10">
      ...
      <div className="p-5 relative">
              ...
              <p className="text-xs text-gray-300 mt-0.5 font-medium">
                ...
                {vehicle.specs.color && (
                  <span className="ml-1.5 inline-flex items-center gap-1">
                    <span className="w-2 h-2 rounded-full border border-gray-200" style={{ background: vehicle.specs.color.toLowerCase() }} />
                    {vehicle.specs.color}
                  </span>
                )}
              </p>
        ...
        {/* Specs */}
        <div className="flex items-center gap-3 mt-4 text-xs text-gray-300 font-medium">
        ...
        <div className="flex items-end justify-between mt-5 pt-4 border-t border-gray-200">
          <div>
            ...
            <span className="text-sm text-gray-400 font-medium ml-0.5">/day</span>
          </div>
          <button
            onClick={(e) => {
              e.stopPropagation();
