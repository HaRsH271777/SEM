 ...
 */
export default function DragCarousel({ children, className = '', title, subtitle, autoScroll = false }: Props) {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      if (scrollRef.current) {
        const { scrollLeft: sl, scrollWidth, clientWidth } = scrollRef.current;
        // If reached end, scroll back to start slowly or instantly
        if (sl + clientWidth >= scrollWidth - 2) {
  ...
  return (
    <div className={className} onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)}>
        ...
        <div className="flex items-end justify-between mb-6">
          ...
          <div className="flex gap-2">
            <button
              ...
              className="w-10 h-10 rounded-full border border-white/10 flex items-center justify-center text-white/60 hover:bg-white/10 hover:text-white transition-all disabled:opacity-30 disabled:hover:bg-transparent"
      ...
      <div className="relative group">
        ...
        <div
          ref={scrollRef}
          className={`flex gap-5 overflow-x-auto scrollbar-hide snap-x snap-mandatory pb-4 ${isDragging ? 'cursor-grabbing select-none' : 'cursor-grab'}`}
